...
function scandi_get_admin_page() {
	global $pagenow;
...
function scandi_default_tab() {
	$current_page = scandi_get_admin_page();
...
function scandi_get_the_tab() {
	global $pagenow;
	
	$default_tab = scandi_default_tab();
...
function scandi_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'scandi') . '</a>';
		}
