...
function scandi_woocommerce_shortcodes($shortcodes) {
...
add_filter('cmsmasters_custom_shortcodes_filter', 'scandi_woocommerce_shortcodes');
...
function cmsmasters_selected_products($atts, $content = null) {
	...
	$out = '<div class="cmsmasters_selected_products_shortcode' . (($categories == true) ? ' show_categories' : '') . 
	(($classes != '') ? ' ' . $classes : '') . 
	(($rating != '') ? esc_attr('show_rating') : '') . 
	'">';
...
function cmsmasters_product_category($atts, $content = null) {
	...
	if (scandi_locate_template($shortcode_path)) {
