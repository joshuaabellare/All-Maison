...
function scandi_woocommerce_register_styles_scripts() {
	global $post;
	
	// Styles
	wp_enqueue_style('scandi-woocommerce-style', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-style.css', array(), '1.0.0', 'screen');
	
	wp_enqueue_style('scandi-woocommerce-adaptive', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-adaptive.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('scandi-woocommerce-rtl', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-rtl.css', array(), '1.0.0', 'screen');
	}
	
	
	// Scripts
	wp_enqueue_script('scandi-woocommerce-script', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/js/jquery.plugin-script.js', array('jquery'), '1.0.0', true);
	...
	$cmsmasters_image_sizes = scandi_get_image_thumbnail_list();
	
	wp_localize_script('scandi-woocommerce-script', 'cmsmasters_woo_script', array( 
...
add_action('wp_enqueue_scripts', 'scandi_woocommerce_register_styles_scripts');


/* Scripts for Admin */
function scandi_woocommerce_admin_scripts() {
	wp_localize_script('scandi-settings-toggle', 'cmsmasters_woocommerce_settings', array(
		'shortname' => 	'scandi'
	));
}

add_action('admin_enqueue_scripts', 'scandi_woocommerce_admin_scripts');


/* Register Default Woocommerce Theme Sidebar */
function scandi_woocommerce_the_widgets_init() {
	...
	register_sidebar(
		array(
			'name' => esc_html__('Shop Sidebar', 'scandi'), 
			'id' => 'sidebar_shop', 
			'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on shop page.', 'scandi'), 
...
add_action('widgets_init', 'scandi_woocommerce_the_widgets_init');


function scandi_woocommerce_demo_store($html, $notice) {
...
add_filter('woocommerce_demo_store', 'scandi_woocommerce_demo_store', 10, 2);


function scandi_woocommerce_support() {
...
add_action('after_setup_theme', 'scandi_woocommerce_support');
