...
if (
	$cmsmasters_option['scandi' . '_portfolio_project_like'] || 
	$cmsmasters_option['scandi' . '_portfolio_project_date'] || 
	$cmsmasters_option['scandi' . '_portfolio_project_cat'] || 
	$cmsmasters_option['scandi' . '_portfolio_project_comment'] || 
	$cmsmasters_option['scandi' . '_portfolio_project_author'] || 
	$cmsmasters_option['scandi' . '_portfolio_project_tag'] || 
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_project'); ?>>
	...
	if ($cmsmasters_post_format == 'gallery') {
		...
		scandi_post_type_gallery(get_the_ID(), $cmsmasters_project_images, $cmsmasters_project_columns, 'cmsmasters-project-thumb', 'cmsmasters-project-full-thumb');
		...
		scandi_post_type_video(get_the_ID(), $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, 'cmsmasters-project-full-thumb');
		...
		if ($cmsmasters_project_image_show == 'true' && $cmsmasters_project_images[0] == '') {
			...
			scandi_post_type_slider(get_the_ID(), $cmsmasters_project_images, 'cmsmasters-full-masonry-thumb');
		}
	}
	...
	if ($cmsmasters_project_title == 'true') {
		echo '<header class="cmsmasters_project_header entry-header">';
			scandi_project_title_nolink(get_the_ID(), 'h1');
		echo '</header>';
	}
			...
			echo '<div class="project_sidebar">';
				...
				if ($project_details == 'true') {
					echo '<div class="project_details entry-meta">' . 
						...
						scandi_get_project_likes('post');
						
						scandi_get_project_comments('post');
						
						scandi_get_project_author('post');
						
						scandi_get_project_date('post');
						
						scandi_get_project_category(get_the_ID(), 'pj-categs', 'post');
						
						scandi_get_project_tags(get_the_ID(), 'pj-tags');
						
						scandi_get_project_features('details', $cmsmasters_project_features, false, 'h3', true);
						
						scandi_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
						
					echo '</div>';
				}
				
				
				scandi_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h3', true);
				
				scandi_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h3', true);
				
				scandi_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h3', true);
...
if ($cmsmasters_option['scandi' . '_portfolio_project_nav_box']) {
	$order_cat = (isset($cmsmasters_option['scandi' . '_portfolio_project_nav_order_cat']) ? $cmsmasters_option['scandi' . '_portfolio_project_nav_order_cat'] : false);
	
	scandi_prev_next_posts(__('Project', 'scandi'), $order_cat, 'pj-categs');
}
...
if ($cmsmasters_project_more_posts != 'hide') {
	scandi_related( 
		'h2', 
		esc_html__('More projects', 'scandi'), 
		esc_html__('No projects found', 'scandi'), 
		$cmsmasters_project_more_posts, 
		$tgsarray, 
		$cmsmasters_option['scandi' . '_portfolio_more_projects_count'], 
		$cmsmasters_option['scandi' . '_portfolio_more_projects_pause'], 
		'project', 
		'pj-tags'
	);
}
